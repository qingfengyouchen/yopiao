...
import com.zx.stlife.service.service.ConvenienceService;
/**
 * 便民服务
 * @author fan
 *
 */
@Controller
@RequestMapping(value = "/service/convenience")
public class ConvenienceController extends BaseController {

	@Autowired
	private ConvenienceService service;

	private static final String PERMISSION_PREFIX = "service:convenience";
	private static final String VIEW_URI_PREFIX = "service/convenience/convenience-";
	...
	public String list(Model model, Page<Convenience> page,
	...
	public String listContent(Model model, Page<Convenience> page,
			ParamsEntity paramsEntity) {
	...
	public String save(
			@Valid @ModelAttribute("preloadEntity") Convenience entity,
	...
	public String edit(@PathVariable("id") Integer id, Model model,
			ParamsEntity paramsEntity) {
		Convenience convenience = service.findOne(id);
		User user = UserUtils.getCurrentUser();
		convenience.setEditor(user.getUserName());
		convenience.setEditTime(new Date());
		model.addAttribute("entity", convenience);
		bindData(model);
		return VIEW_URI_PREFIX + "form";
	}

	private void bindData(Model model) {
		model.addAttribute("shareGoodsList", service.findByState());
	}
	...
	public Convenience getEntity(@RequestParam(value = "id", required = false) Integer id) {
		if ( null != id ) {
	...
	public String convenienceList(Page<Convenience> page, Model model) {
		service.searchAll(page);
		model.addAttribute("convenience", Const.CONVENIENCE)
