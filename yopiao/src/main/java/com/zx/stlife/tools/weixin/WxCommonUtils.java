...
public class WxCommonUtils {
	private static Logger log = LoggerFactory.getLogger(WxCommonUtils.class);
	...
	public static String httpsRequestWithCert(String requestUrl, String outputStr) {
		...
		try {
			...
			instream = new FileInputStream(new File(Const.CERT_PATH));
			keyStore.load(instream, Const.CERT_KEY.toCharArray());
			SSLContext sslcontext = SSLContexts.custom().loadKeyMaterial(keyStore, Const.CERT_KEY.toCharArray())
					.build();
