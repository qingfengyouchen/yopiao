...
(function( $, undefined ) {
	...
	$.widget( "cg.combogrid", {
		options: {
			...
			showOn:false,
			width:null,
			otherParamsId: null //其他参数Id 隐藏域元素Id
		},
		source: null,
		lastOrdered: "",
		cssCol: "",
			...
			this.options.source =  function( request, response ) {
				$.ajax({
					...
					data: {
						...
						otherParams: $("#" + self.options.otherParamsId).val()
					},
					success: function( data ) {
						if(self.options.matchResultCallback){
							self.options.matchResultCallback(data.records > 0 && $.trim(request.term).length > 0);
						}
						if(data.records==0) {
							self.pending--;
							if ( !self.pending ) {
			...
			this.menucombo = $( "<div></div>" )
				...
				.menucombo({
					...
					selected: function( event, ui ) {
						...
						self.stopListenChange();
						if ( false !== self._trigger( "select", event, { item: item }, 1111) ) {
							self.element.val( item.value );
						}
						self.startListenChange();
						...
						if(self.options.matchResultCallback){
							self.options.matchResultCallback(true);
						}
					},
		...
		startListenChange: function(){
			var self = this;
			self.originValue = self.element.val();
			self.listerChange = setInterval(function(){
				var value = self.element.val();
				if(self.originValue != value){
					self.originValue = value;
					self._search(value);
				}
			}, self.options.delay);
		},
		stopListenChange: function(){
			var self = this;
			clearInterval(self.listerChange);
		},
		...
		close: function( event ) {
			...
			if ( this.menucombo.element.is(":visible") ) {
				...
				this.options.orderBy=self.options.orderBy;
				this.cssCol="";
				this.lastOrdered="";
...
//		ul2.appendTo(ul);
			...
			$.each( colModel, function( index, col ) {
				...
				if(col.columnName==self.cssCol){
					div.append('<div class="cg-colHeader" style="width:'+col.width+self.options.munit+';'+hide+' text-align:'+col.align+'"><label class="cg-colHeader-label" id="'+ col.columnName +'">'
						...
						+'</label><span class="cg-colHeader '+ self.options.order +'"></span></div>');
				} else {
					div.append('<div class="cg-colHeader" style="width:'+col.width+self.options.munit+';'+hide+' text-align:'+col.align+'"><label class="cg-colHeader-label" id="'+ col.columnName +'">'
						+self._renderLabel(col.label)
						+'</label></div>');
		...
		_renderPager: function( ul, records, total ) {
			var self = this;
			var initRecord = ((self.pageNo*self.options.pageSize)-self.options.pageSize)+1;
			var lastRecord = 0;
			if (self.pageNo<total){
				lastRecord = (self.pageNo*self.options.pageSize);
			} else {
				lastRecord = records;
			}
			...
			$("<table cellspacing='0' cellpadding='0' border='0' class='cg-navTable'>"
				...
				+self._renderPagerPage('pageNo', self.pageNo, total)
				+"</td>"
				+"<td style='width: 4px;' class='cg-state-disabled'>"
				+"<span class='ui-separator'></span>"
				+"</td>"
...
// Select pageNo
			...
			$('.' +self.element.attr('id') +'.recordXP').val(self.options.pageSize);
			if(self.pageNo>1){
		...
		_renderPagerPage: function (label, pageNo, total){
			var self = this;
			if(this.options.i18n){
				return $.i18n.prop('pageNo')+' <input type="text" size="1" class="' +self.element.attr('id')+ ' currentPage" value="'+ pageNo + '"></input> ' + $.i18n.prop('of') + ' ' + total;
			} else {
				return 'pageNo <input type="text" size="1" class="' +self.element.attr('id')+ ' currentPage" value="' + pageNo + '"></input> of ' + total;
			}
		},
	...
	$.widget("cg.menucombo", {
		...
		nextPage: function(event) {
			if (this.hasScroll()) {
				...
				// TODO try to catch this earlier when scrollTop indicates the last pageNo anyway
				if (!result.length) {
