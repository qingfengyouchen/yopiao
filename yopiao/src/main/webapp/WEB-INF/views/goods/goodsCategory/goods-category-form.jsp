		...
		<div class="col-sm-12">
			<div class="ibox float-e-margins">
				<div class="ibox-title">
					<h5>
						商品类别管理 > ${empty entity.id ? "新增" : "修改"}商品类别
					</h5>
				</div>
				<div class="ibox-content">
					<form:form id="inputForm" modelAttribute="entity" action="${ctx}/goods/goodsCategory/save" method="post" class="form-horizontal">
						<form:hidden path="id"/>
								...
								<div id="fileList2" class="uploader-list">
									<c:if test="${not empty entity.imgUrl}">
										<div class="file-item thumbnail" imgId="${entity.id}">
											<img src="${goodsCategoryRootUri}/${entity.imgUrl}" style="width:135px;">
	...
	<script>
			...
			$("#inputForm").validate({
				rules: {
					name: {
						rangelength: [1, 20],
						required: true,
						remote: "${ctx}/goods/goodsCategory/checkName?oldName=" + encodeURIComponent('${entity.name}')
					},
					sortNum: {
						numRange: [1, 99]
					}
		...
		function delImg(_this, type){
			...
			if(imgId || tmpFileId){
				if(confirm('确定要删除图片吗？')){
					$.ajax({
						url: '${ctx}/goods/goodsCategory/deleteImg',
						data:{
							type: type,
							imgId: imgId,
							tmpFileId: tmpFileId
